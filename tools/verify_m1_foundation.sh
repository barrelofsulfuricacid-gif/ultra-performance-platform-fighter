...
grep -q '^sim-contract=pass abi=3 tick_hz=60$' \
...
grep -q '^headless-smoke=pass sim_abi=3 tick_hz=60$' \
...
if [ -n "$cmake_command" ]; then
    ...
    run_cmake_smoke \
        headless \
        "headless-smoke=pass sim_abi=3 tick_hz=60" \
        headless.txt
    run_cmake_smoke \
        pf_native_client \
        "native-client-smoke=pass sim_abi=3 tick_hz=60" \
        native_client.txt
    run_cmake_smoke \
    ...
    run_cmake_smoke \
        pf_tools \
        "tools-smoke=pass sim_abi=3 tick_hz=60" \
        tools.txt
    run_cmake_smoke \
        pf_benchmarks \
        "benchmarks-smoke=pass sim_abi=3 tick_hz=60" \
        benchmarks.txt
    run_cmake_smoke \
        pf_verifier \
        "verifier-smoke=pass sim_abi=3 tick_hz=60" \
        verifier.txt

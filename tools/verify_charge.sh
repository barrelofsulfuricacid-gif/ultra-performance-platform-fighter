...
compiler=${CC:-cc}
expected='m4-charge=pass content_schema=78 state_schema=78 save_bytes=1787 charge_invariants=28 charge_storage_cancel=1 resumed_release=1 replay=1 rl=1'
...
"$compiler" \
    ...
    "$root/tests/sim/test_charge.c" \
    -o "$output_dir/charge_test"

actual=$($output_dir/charge_test)
printf '%s\n' "$actual"
...
grep -Fq 'prepare_charge_input' "$root/src/sim/sim_tick.c"

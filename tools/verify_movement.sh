...
output_dir=${1:-"$root/performance/local/movement"}
compiler=${CC:-cc}
...
"$compiler" $common_flags \
    ...
    "$root/tests/sim/test_movement.c" \
    -o "$output_dir/movement_test"

"$output_dir/movement_test" >"$output_dir/movement.txt"
grep -Fqx \
    'm4-movement=pass content_schema=74 deterministic_ticks=20000 movement_core=pass tap_jump=1 jump_takeoff_momentum=1 player_push=1 teeter_cancel=1 taunt_cancel=1 double_jump_cancel=1 vector_ascent=1 ledge_roll=1 emergent_technique_tests=skipped' \
    "$output_dir/movement.txt"

...
iso="$1"
decomp_root="$2"
output="${3:-${repo_root}/build/oracle/falcon-slope-ledge-response-checkpoint.json}"
sim_executable="${4:-}"
...
if [[ -z "${warm_seconds}" ]]; then
    echo "slope/ledge checkpoint pack did not report a warm duration" >&2
    exit 1
fi
...
if [[ -n "${sim_executable}" ]]; then
    ...
    "${sim_executable}" \
        --ssbm-oracle falcon-common-slope-ledge-response > "${sim_output}"
    sim_arguments=(--sim-output "${sim_output}")
fi

common_root="${decomp_root}/src/melee/ft/chara/ftCommon"
"${python}" "${repo_root}/tools/verify_ssbm_falcon_slope_ledge_response.py" \
    "${output}" \
    "${coverage_manifest}" \
    "${stage_source}" \
    "${common_root}/ftCo_DownBound.c" \
    "${common_root}/ftCo_Down.c" \
    "${common_root}/ftCo_Escape.c" \
    "${common_root}/ftCo_Fall.c" \
    "${decomp_root}/src/melee/ft/ft_081B.c" \
    "${decomp_root}/src/melee/mp/mpcoll.c" \
    "${decomp_root}/src/melee/ft/ftcliffcommon.c" \
    "${decomp_root}/src/melee/ft/fighter.c" \
    "${decomp_root}/src/melee/ft/ftcommon.c" \
    "${sim_arguments[@]}"

printf 'ssbm-falcon-slope-ledge-response-pack=pass warm_seconds=%s output=%s\n' \
    "${warm_seconds}" "${output}"

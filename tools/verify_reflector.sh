...
compiler=${CC:-cc}
expected='m4-reflector=pass content_schema=78 state_schema=78 save_bytes=1787 reflector_invariants=32 shine_spike=1 projectile_reflect=1 replay=1 rl=1'
...
"$compiler" \
    ...
    "$root/tests/sim/test_reflector.c" \
    -o "$output_dir/reflector_test"

actual=$($output_dir/reflector_test)
printf '%s\n' "$actual"
...
grep -Fq 'prepare_reflector_input' "$root/src/sim/sim_tick.c"

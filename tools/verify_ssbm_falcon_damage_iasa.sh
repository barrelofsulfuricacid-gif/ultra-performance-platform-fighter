...
iso="$1"
decomp_root="$2"
output="${3:-${repo_root}/build/oracle/falcon-damage-iasa.json}"
sim_executable="${4:-}"
manifest="${repo_root}/tools/ssbm_falcon_damage_iasa_coverage.json"
...
warm_seconds="$(
    sed -n 's/.*warm_seconds=\([0-9.]*\).*/\1/p' "${timing_log}" | tail -n 1
)"
warm_budget_seconds="$(
    "${python}" -c \
...
if [[ -z "${warm_seconds}" ]]; then
    echo "damage-IASA checkpoint pack did not report a warm duration" >&2
    exit 1
fi
awk -v seconds="${warm_seconds}" -v budget="${warm_budget_seconds}" \
    'BEGIN { exit !(seconds <= budget) }' || {
    echo "damage-IASA pack exceeded ${warm_budget_seconds}-second warm budget: ${warm_seconds}" >&2
    exit 1
}
...
if [[ -n "${sim_executable}" ]]; then
    ...
    "${sim_executable}" --ssbm-oracle falcon-common-damage-iasa > "${sim_output}"
    sim_arguments=(--sim-output "${sim_output}")
fi

common_root="${decomp_root}/src/melee/ft/chara/ftCommon"
"${python}" "${repo_root}/tools/verify_ssbm_falcon_damage_iasa.py" \
    "${output}" \
    "${manifest}" \
    "${common_root}/ftCo_Damage.c" \
    "${common_root}/ftCo_DamageFall.c" \
    "${common_root}/ftCo_EscapeAir.c" \
    "${sim_arguments[@]}"

printf 'ssbm-falcon-damage-iasa-pack=pass warm_seconds=%s output=%s\n' \
    "${warm_seconds}" "${output}"

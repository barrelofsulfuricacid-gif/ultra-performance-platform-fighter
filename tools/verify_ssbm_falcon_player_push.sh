...
iso="$1"
decomp_root="$2"
output="${3:-${repo_root}/build/oracle/falcon-player-push-checkpoint.json}"
sim_executable="${4:-}"
coverage_manifest="${repo_root}/tools/ssbm_falcon_player_push_coverage.json"
...
if [[ -z "${warm_seconds}" ]]; then
    echo "player-push checkpoint pack did not report a warm duration" >&2
    exit 1
fi
awk -v seconds="${warm_seconds}" -v budget="${warm_budget_seconds}" \
    'BEGIN { exit !(seconds <= budget) }' || {
    echo "player-push pack exceeded ${warm_budget_seconds}-second warm budget: ${warm_seconds}" >&2
    exit 1
}
...
if [[ -n "${sim_executable}" ]]; then
    ...
    "${sim_executable}" --ssbm-oracle falcon-common-player-push > "${sim_output}"
    sim_arguments=(--sim-output "${sim_output}")
fi

"${python}" "${repo_root}/tools/verify_ssbm_falcon_player_push.py" \
    "${output}" \
    "${coverage_manifest}" \
    "${decomp_root}/src/melee/ft/ftcommon.c" \
    "${sim_arguments[@]}"

printf 'ssbm-falcon-player-push-pack=pass warm_seconds=%s output=%s\n' \
    "${warm_seconds}" "${output}"

...
def main() -> int:
    ...
    coordinate_scale_f32 = 65536.0 * float(numerator) / float(denominator)
    ...
    for case in oracle["cases"]:
        ...
        rendered_cases.append(
            (case, round(float(frame) * 65536.0), capsules, pose_f32(ecb))
        )

    lines = [
        "/* Generated by tools/generate_ssbm_dynamic_hurt_oracle.py. */",
        f"#define {macro_prefix}_TOLERANCE_Q16 INT32_C({int(oracle['coordinate_tolerance_f32'])})",

...
from ssbm_ecb_pose import pose_f32
...
def main() -> int:
    ...
    macro_prefix = "PF_M4_" + symbol_prefix.upper() + "_ORACLE"
    ...
    for case in oracle["cases"]:
        ...
        capsules = canonical_hurt_pose_f32(
            memory,
            "fighter_hurtboxes",
            "fighter_position",
            int(facing),
        ...
        rendered_cases.append(
            (case, binary32(float(frame)), capsules, pose_f32(ecb))
        )

    lines = [
        "/* Generated by tools/generate_ssbm_dynamic_hurt_oracle.py. */",
        f"#define {macro_prefix}_TOLERANCE_F32 {c_f32(float(oracle['coordinate_tolerance_f32']))}",
    ...
    for case, frame_f32, capsules, ecb in rendered_cases:
        lines.append(
        ...
        for capsule in capsules:
            lines.append(
                "        { " + ", ".join(c_f32(float(value)) for value in capsule[:7]) +
        ...
        lines.append(
            "    }, { " + ", ".join(c_f32(value) for value in ecb_values) +
            " } },"
        )
    lines.extend(["};", ""])
    output = "\n".join(lines)

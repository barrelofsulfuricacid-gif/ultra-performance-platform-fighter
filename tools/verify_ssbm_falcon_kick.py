#!/usr/bin/env python3
"""Verify Falcon Kick's stored projection against live SSBM captures."""
...
def fail(message: str) -> None:
    raise SystemExit(f"ssbm-falcon-kick-source=fail reason={message}")
...
def load_capture(
    ...
    if (
        ...
        or capture.get("stage") != expected_stage
...
def select_route_rows(
    ...
    start_label = f"special_geometry_down_{case_id}_start"
    ...
    if case_id in {"ground", "ground_hit", "air_land"}:
        end_action = "STANDING"
        end = next(
            (
                index
                for index, row in enumerate(selected)
                if index > 0 and row.get("action") == end_action
            ),
            None,
        )
    elif case_id in {"ground_edge", "air"}:
        end_action = "FALLING"
        end = next(
            (
                index
    ...
    elif case_id == "ground_wall":
        end_action = "SWORD_DANCE_3_LOW_AIR"
        matches = [
            index
            for index, row in enumerate(selected)
            if row.get("action") == end_action
        ]
        end = max(matches) if matches else None
    else:
        fail(f"unknown-route case={case_id}")
    if end is None:
        fail(f"route-end case={case_id} action={end_action}")
...
def source_sample_values(
    ...
    velocity_x_key = (
        ...
        if (
            route == "ground_hit"
            and float(row.get("opponent_damage_percent", 0.0)) > 0.0
        )
        or not bool(row["grounded"])
        or action == "FALLING"
        else "ground_velocity_x"
    )
    horizontal_mirror = -1 if route == "air" else 1
    return {
        "action_state": falcon_kick_expected_action(
            route, action, action_frame, hitlag_left
        ),
        "action_ticks": falcon_kick_expected_ticks(
            route, action, action_frame, hitlag_left
        ),
...
def main() -> int:
    ...
    parser.add_argument("air_land_capture", type=Path)
    parser.add_argument("ground_edge_capture", type=Path)
    parser.add_argument("ground_hit_capture", type=Path)
    parser.add_argument("ground_wall_capture", type=Path)
    args = parser.parse_args()

    coverage = json.loads(args.coverage.read_text(encoding="utf-8"))
    if coverage.get("schema") != 1 or coverage.get("domain") != "falcon-down-special":
        fail("coverage-manifest")
    live_source = coverage.get("live_source", {})
    stored = coverage.get("stored_oracle", {})
    if not isinstance(live_source, dict) or stored.get("kind") != "native-csv-trace-v1":
        fail("coverage-schema")

    paths = {
        "ground": args.ground_capture,
        "air": args.air_capture,
        "air_land": args.air_land_capture,
        "ground_edge": args.ground_edge_capture,
        "ground_hit": args.ground_hit_capture,
        "ground_wall": args.ground_wall_capture,
    }
    captures = {
        case_id: load_capture(
            path,
            str(live_source[f"{case_id}_capture_sha256"]),
            "HYRULE_TEMPLE" if case_id == "ground_wall" else "FINAL_DESTINATION",
            live_source,
        )
    ...
    for case in raw_cases:
        ...
        if len(rows) != sample_count:
            fail(
                f"sample-count case={case_id} expected={sample_count} actual={len(rows)}"
            )
        ...
        if live_input_lines != stored_input_lines:
            ...
            fail(
                ...
                f"live={live_input_lines[mismatch]} stored={stored_input_lines[mismatch]}"
            )
        ...
        anchor_x = (
            float(rows[0]["position_x_from_origin"])
            if case_id in {"ground_edge", "ground_hit", "ground_wall"}
            else 0.0
        )
        anchor_y = (
            float(rows[0]["position_y"])
            - float(rows[0]["velocity_y"])
            if case_id == "air_land"
            else float(rows[0]["position_y"])
            if case_id == "ground_wall"
            else 0.0
        )
        ...
        for sample_index, row in enumerate(rows):
            ...
            if (
                case_id == "ground_wall"
                and action == "SWORD_DANCE_3_LOW_AIR"
                and previous_action != action
            ):
                anchor_x = float(row["position_x_from_origin"])
                anchor_y = float(row["position_y"])
            retained_shield_strength = normalized_shield_strength(
                row, retained_shield_strength
            )
            values = source_sample_values(
                row, case_id, anchor_x, anchor_y, retained_shield_strength
            )
            selected_fields = selected_trace_fields(
                fields, exclusions, sample_index
            )
            ...
            for field in selected_fields:
                ...
                if not isinstance(value, int):
                    fail(
                        f"source-field case={case_id} sample={sample_index} field={field}"
                    )
                sample[field] = value

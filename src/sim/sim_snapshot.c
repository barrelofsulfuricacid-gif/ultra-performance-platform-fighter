...
#define PF_SIM_SAVE_PAYLOAD_BYTES ((size_t)1660)
#define PF_SIM_SAVE_TOTAL_BYTES \
    (PF_SIM_SAVE_HEADER_BYTES + PF_SIM_SAVE_PAYLOAD_BYTES)
#define PF_M4_SNAPSHOT_GROUND_BLEND_REPLAY_TAG UINT32_C(0x80000000)
#define PF_M4_SNAPSHOT_GROUND_BLEND_PROGRESS_MASK UINT32_C(0x7fffffff)
...
static void pf_writer_f32(pf_byte_writer *writer, float value)
{
    uint32_t bits;

    (void)memcpy(&bits, &value, sizeof(bits));
    pf_writer_u32(writer, bits);
}
...
static float pf_reader_f32(pf_byte_reader *reader)
{
...
static uint16_t snapshot_canonical_source_submotion(
    ...
    uint32_t player_index)
{
    ...
    return action_retains_source_submotion(
...
static float snapshot_canonical_source_animation_frame_f32(
    const pf_world_state *world,
    uint32_t player_index)
{
...
static float snapshot_canonical_source_animation_rate_f32(
    const pf_world_state *world,
    uint32_t player_index)
{
    return world->active[player_index] != UINT8_C(0) &&
...
static float snapshot_canonical_fall_animation_blend_f32(
    const pf_world_state *world,
    uint32_t player_index)
{
    return world->active[player_index] != UINT8_C(0) &&
...
static uint8_t snapshot_canonical_fall_animation_target_switched(
    const pf_world_state *world,
    uint32_t player_index)
{
    return snapshot_canonical_fall_animation_blend_f32(
               world, player_index) != INT32_C(0)
    ...
    const pf_world_state *world)
{
    ...
    pf_writer_f32(writer, world->arena_half_width_f32);
    pf_writer_f32(writer, world->arena_ceiling_f32);
    ...
    pf_writer_u8(writer, world->reference_match_input_lock_ticks);
    ...
    for (player_index = UINT32_C(0);
         ...
         ++player_index)
    {
        pf_writer_f32(writer, world->position_x_f32[player_index]);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
    {
        pf_writer_f32(writer, world->position_y_f32[player_index]);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
    {
        pf_writer_f32(writer, world->velocity_x_f32[player_index]);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
    {
        pf_writer_f32(writer, world->velocity_y_f32[player_index]);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
         ...
         ++player_index)
    {
        pf_writer_u16(
            writer,
            snapshot_canonical_source_submotion(
                world,
                player_index));
    }
    for (player_index = UINT32_C(0);
    ...
    {
        const int active =
            world->ground_blend_progress_f32[player_index] > INT32_C(0);
            ...
            active != 0
                ? pf_sim_f32_bits(
                      ...
                      (world->ground_blend_pose[player_index].mode ==
                           ...
                           ? PF_M4_SNAPSHOT_GROUND_BLEND_REPLAY_TAG
                           : UINT32_C(0))
                : UINT32_C(0));
             ...
             ++translation_index)
        {
            ...
            for (component = UINT8_C(0); component < UINT8_C(3); ++component)
            {
                pf_writer_f32(
                    ...
                    active != 0
                        ? world->ground_blend_pose[player_index]
                              .translation_f32[translation_index][component]
                        : INT32_C(0));
            }
        }
    }
         ...
         ++player_index)
    {
        pf_writer_f32(
            writer,
            snapshot_canonical_source_animation_frame_f32(
                world,
                player_index));
    }
    for (player_index = UINT32_C(0);
         ...
         ++player_index)
    {
        pf_writer_f32(
            writer,
            snapshot_canonical_source_animation_rate_f32(
                world,
                player_index));
    }
    for (player_index = UINT32_C(0);
         ...
         ++player_index)
    {
        pf_writer_f32(
            writer,
            snapshot_canonical_fall_animation_blend_f32(
                world,
                player_index));
    }
    for (player_index = UINT32_C(0);
         ...
         ++player_index)
    {
        pf_writer_u8(
            writer,
            snapshot_canonical_fall_animation_target_switched(
                world,
                player_index));
    }
    for (player_index = UINT32_C(0);
    ...
    for (player_index = UINT32_C(0);
         ...
         ++player_index)
    {
        pf_writer_f32(writer, world->damage_f32[player_index]);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
    ...
    for (player_index = UINT32_C(0);
         ...
         ++player_index)
    {
        pf_writer_f32(writer, world->shield_health_f32[player_index]);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
    ...
    for (player_index = UINT32_C(0);
         ...
         ++player_index)
    {
        pf_writer_u8(writer, world->up_special_input_age[player_index]);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
    ...
    pf_writer_f32(writer, world->item_position_x_f32);
    pf_writer_f32(writer, world->item_position_y_f32);
    pf_writer_f32(writer, world->item_velocity_x_f32);
    pf_writer_f32(writer, world->item_velocity_y_f32);
    ...
    pf_writer_f32(writer, world->projectile_position_x_f32);
    pf_writer_f32(writer, world->projectile_position_y_f32);
    pf_writer_f32(writer, world->projectile_velocity_x_f32);
    pf_writer_f32(writer, world->projectile_velocity_y_f32);
    ...
    pf_world_state *world)
{
    ...
    world->arena_half_width_f32 = pf_reader_f32(reader);
    world->arena_ceiling_f32 = pf_reader_f32(reader);
    ...
    world->reference_match_input_lock_ticks = pf_reader_u8(reader);
    ...
    for (player_index = UINT32_C(0);
         ...
         ++player_index)
    {
        world->position_x_f32[player_index] = pf_reader_f32(reader);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
    {
        world->position_y_f32[player_index] = pf_reader_f32(reader);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
    {
        world->velocity_x_f32[player_index] = pf_reader_f32(reader);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
    {
        world->velocity_y_f32[player_index] = pf_reader_f32(reader);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
    ...
    {
        world->shield_recoil_x_f32[player_index] =
            pf_reader_f32(reader);
        if (world->shield_recoil_x_f32[player_index] != INT32_C(0))
        {
         ...
         ++player_index)
    {
        ...
        const uint32_t encoded_progress = pf_reader_u32(reader);

        if ((encoded_progress & PF_M4_SNAPSHOT_GROUND_BLEND_PROGRESS_MASK) >
            UINT32_C(0x7f7fffff))
        {
            reader->failed = 1;
        }
        ...
        world->ground_blend_progress_f32[player_index] =
            pf_sim_f32_from_bits(
                encoded_progress &
                PF_M4_SNAPSHOT_GROUND_BLEND_PROGRESS_MASK);
        for (rotation_index = UINT8_C(0);
             rotation_index < PF_M4_HSD_COMPACT_ROTATION_CAPACITY;
             ++rotation_index)
        {
         ...
         ++player_index)
    {
        world->ecb_locked_bottom_y_f32[player_index] =
            pf_reader_f32(reader);
        world->ecb_bottom_lock_ticks[player_index] = pf_reader_u8(reader);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
    ...
    for (player_index = UINT32_C(0);
         ...
         ++player_index)
    {
        world->damage_f32[player_index] = pf_reader_f32(reader);
    }
    for (player_index = UINT32_C(0);
         player_index < PF_SIM_MAX_PLAYERS;
         ++player_index)
    ...
    world->item_position_x_f32 = pf_reader_f32(reader);
    world->item_position_y_f32 = pf_reader_f32(reader);
    world->item_velocity_x_f32 = pf_reader_f32(reader);
    world->item_velocity_y_f32 = pf_reader_f32(reader);
    ...
    world->projectile_position_x_f32 = pf_reader_f32(reader);
    world->projectile_position_y_f32 = pf_reader_f32(reader);
    world->projectile_velocity_x_f32 = pf_reader_f32(reader);
    world->projectile_velocity_y_f32 = pf_reader_f32(reader);
    ...
    const pf_world_state *right)
{
    return pf_hash_equal(
           ...
           left->arena_half_width_f32 == right->arena_half_width_f32 &&
           left->arena_ceiling_f32 == right->arena_ceiling_f32 &&
...
static int snapshot_action_is_surface_tech(uint8_t action)
{
...
static int snapshot_action_is_throw(uint8_t action)
{
...
static int snapshot_action_is_aerial_attack(uint8_t action)
{
...
static int snapshot_action_is_ground_attack(uint8_t action)
{
    return action == (uint8_t)PF_M4_ACTION_GROUND_ATTACK ||
           ...
           action_is_reference_angled_normal(action) ||
...
static int snapshot_action_is_landing(uint8_t action)
{
...
static int snapshot_action_is_reference_air_special(uint8_t action)
{
...
static int snapshot_action_is_ground_falcon_kick(uint8_t action)
{
    return action ==
               (uint8_t)PF_M4_ACTION_FALCON_KICK_START_GROUND ||
           action ==
...
static int snapshot_action_is_ground_falcon_dive_start(
    uint8_t action,
...
static int snapshot_action_is_falcon_dive_capture_holder(
    uint8_t action,
...
static int snapshot_action_is_grabbed(
    uint8_t action,
    uint8_t resume_action)
{
...
static int snapshot_action_is_normal_grab_holder(
    uint8_t action,
    uint8_t resume_action)
{
    const uint8_t effective_action =
...
static int snapshot_action_is_throw_holder(
    uint8_t action,
    uint8_t resume_action)
{
    return snapshot_action_is_throw(
        action == (uint8_t)PF_M4_ACTION_HITLAG
            ? resume_action
            : action);
}

static int snapshot_action_is_smash_charge(uint8_t action)
{
    return action ==
...
static int snapshot_action_is_smash_release(uint8_t action)
{
    return action_is_smash_release(action);
}

static uint16_t snapshot_smash_charge_frame(uint8_t action)
{
    falcon_move_index move_index;
    const struct reference_move *move;

    switch ((enum action_state)action)
    {
...
static const attack_data *snapshot_ground_attack_data(
...
static float snapshot_revival_platform_y(
    const stage_data *stage,
    uint16_t action_ticks)
{
    const uint16_t physics_ticks =
        action_ticks < UINT16_MAX
            ? (uint16_t)(action_ticks + UINT16_C(1))
            : action_ticks;
    const uint16_t elapsed =
        physics_ticks < stage->revival_platform_descent_ticks
            ? physics_ticks
    ...
    return stage->revival_platform_start_y_f32 +
           distance * (float)elapsed /
               (float)stage->revival_platform_descent_ticks;
}

static int snapshot_source_submotion_valid_for_action(
    uint8_t action,
    uint8_t resume_action,
    uint16_t submotion,
    uint16_t action_ticks,
    ...
    uint8_t reference_frame_data_enabled)
{
    ...
    const falcon_submotion_data *motion =
        falcon_reference_submotion(submotion);
    int identity_valid = 0;

    if (reference_frame_data_enabled == UINT8_C(0) &&
        (action_uses_ledge(effective_action) ||
         action_uses_source_animation_clock(action, resume_action)))
    {
        return submotion ==
               (uint16_t)PF_M4_FALCON_SUBMOTION_WAIT;
    }
    ...
    {
        return motion != NULL &&
               falcon_reference_wait_animation(submotion) != NULL;
    }
    ...
    else if (effective_action == (uint8_t)PF_M4_ACTION_CROUCH_END)
    {
        return motion != NULL &&
               submotion ==
    ...
    else if (action_uses_direct_hsd_pose(effective_action))
    {
        uint16_t expected_submotion = UINT16_C(0);
        float ignored_frame_f32 = INT32_C(0);
    ...
    else if (action_is_damage(effective_action))
    {
        return reference_frame_data_enabled != UINT8_C(0) &&
               motion != NULL &&
               submotion >=
    ...
    else if (effective_action == (uint8_t)PF_M4_ACTION_GRAB ||
             effective_action == (uint8_t)PF_M4_ACTION_DASH_GRAB)
    {
        const uint16_t expected_submotion =
            effective_action == (uint8_t)PF_M4_ACTION_GRAB
                ? (uint16_t)PF_M4_FALCON_SUBMOTION_CATCH
                : (uint16_t)PF_M4_FALCON_SUBMOTION_CATCH_DASH;

        return reference_frame_data_enabled != UINT8_C(0) &&
               motion != NULL && submotion == expected_submotion &&
               action_ticks != UINT16_C(0) &&
               action_ticks <= motion->animation_frame_count;
    }
    ...
    else if (snapshot_action_is_throw(effective_action))
    {
        ...
        return reference_frame_data_enabled != UINT8_C(0) &&
               falcon_reference_throw_motions(
                   effective_action,
                   &expected_holder_submotion,
                   &victim_submotion,
                   &animation_rate_f32) &&
               submotion == expected_holder_submotion &&
               action_ticks <= UINT16_C(600);
    }
    else if (effective_action == (uint8_t)PF_M4_ACTION_GRABBED)
    {
        if (submotion == (uint16_t)
            ...
            submotion ==
                (uint16_t)PF_M4_FALCON_SUBMOTION_THROWN_BACK ||
            submotion ==
                (uint16_t)PF_M4_FALCON_SUBMOTION_THROWN_UP ||
            submotion ==
...
static int snapshot_source_animation_clock_valid(
    const fighter_data *fighter,
    uint8_t active,
    uint8_t action,
    uint8_t resume_action,
    ...
    float rate_f32)
{
    ...
    if (active == UINT8_C(0) ||
        ...
        !action_uses_source_animation_clock(
            ...
            resume_action))
    {
        return frame_f32 == INT32_C(0) && rate_f32 == INT32_C(0);
    }
    ...
    uint32_t player_index)
{
    ...
    const uint8_t effective_action = effective_action_state(
        world->action_state[player_index],
        world->hitlag_resume_action[player_index]);
    const int guard_on_pose =
        effective_action == (uint8_t)PF_M4_ACTION_SHIELD &&
        world->source_submotion[player_index] ==
            (uint16_t)PF_M4_FALCON_SUBMOTION_GUARD_ON &&
    ...
    if (progress_f32 == INT32_C(0))
    {
             ...
             ++translation_index)
        {
            for (uint8_t component = UINT8_C(0);
                 ...
                 ++component)
            {
                if (compact->translation_f32[translation_index][component] !=
                    INT32_C(0))
                {
                    return 0;
                }
    ...
    }
    {
        ...
        {
            const float *const replay_words =
                &compact->translation_f32[0][0];
            uint8_t replay_word_index;

            if (compact->replay.reserved != UINT16_C(0))
            {
                return 0;
            }
            for (rotation_index = UINT8_C(0);
            ...
            for (replay_word_index = UINT8_C(5);
                 replay_word_index <
                     (uint8_t)(
                         PF_M4_HSD_COMPACT_TRANSLATION_CAPACITY * UINT8_C(3));
                 ++replay_word_index)
            {
                if (replay_words[replay_word_index] != 0.0f)
                {
                    return 0;
                }
            }
...
static int snapshot_fall_animation_blend_valid(
    const fighter_data *fighter,
    const pf_world_state *world,
    uint32_t player_index)
{
    const float blend_f32 =
        world->fall_animation_blend_f32[player_index];
    const uint8_t target_switched =
        world->fall_animation_target_switched[player_index];
    const uint8_t effective_action = effective_action_state(
        world->action_state[player_index],
        world->hitlag_resume_action[player_index]);
...
{
    const falcon_ledge_attack_reference *reference_attack =
        fighter->reference_frame_data_enabled != UINT8_C(0)
         ...
         ++player_index)
    {
        ...
        if (world->active[player_index] != UINT8_C(0) &&
            action_retains_source_submotion(
                ...
                resume_action))
        {
            if (!snapshot_source_submotion_valid_for_action(
                    action,
                    resume_action,
                    source_submotion,
                    action_ticks,
        ...
        if (!snapshot_source_animation_clock_valid(
                &content->fighter,
        ...
        if (!snapshot_ground_blend_valid(
                &content->fighter, world, player_index))
        {
            return 0;
        }
        if (!snapshot_fall_animation_blend_valid(
                &content->fighter, world, player_index))
        {
            return 0;
        }
        ...
        const attack_data *ground_attack =
            snapshot_ground_attack_data(
                &content->fighter,
                action);
        const attack_data *ground_attack_resume =
            snapshot_ground_attack_data(
                &content->fighter,
                resume_action);
        const reference_timing ground_attack_timing =
            ground_attack != NULL
                ? (reference_timing){
                ...
                : (reference_timing){0};
        falcon_move_index ground_attack_move_index;
        const int falcon_ground_attack =
            ...
            (content->fighter.reference_frame_data_enabled != UINT8_C(0) &&
                 ...
                 : falcon_reference_attack_matches(
                       ...
                       ground_attack->damage_f32));
        const uint16_t smash_charge_ticks =
            world->smash_charge_ticks[player_index];
        const int smash_charge_action =
            snapshot_action_is_smash_charge(action);
        const int smash_release_action =
            snapshot_action_is_smash_release(action);
        const int smash_release_resume =
            snapshot_action_is_smash_release(resume_action);
        const uint16_t source_smash_charge_frame =
            content->fighter.reference_frame_data_enabled != UINT8_C(0)
                ? snapshot_smash_charge_frame(action)
                : UINT16_C(0);
        ...
        const float revival_x =
            ssbm_revival_platform_x_f32(
                content->stage.reference_collision_profile,
                world->player_count,
                (uint8_t)player_index,
                content->stage.spawn_spacing_f32);
        const float revival_y =
            snapshot_revival_platform_y(
                &content->stage,
                action_ticks) -
        ...
        if (world->charge_ticks[player_index] >
            ...
            (action == (uint8_t)PF_M4_ACTION_TEETER &&
             world->velocity_x_f32[player_index] != INT32_C(0)) ||
            ...
            (revival_action &&
             ((uint32_t)action_ticks > revival_ticks ||
              world->position_x_f32[player_index] != revival_x ||
              world->position_y_f32[player_index] != revival_y ||
              world->respawn_invulnerability_ticks[player_index] !=
                  UINT16_C(0))) ||
...
static int player_state_consistent(
    const pf_world_state *world,
    uint32_t player_index)
{
    ...
    if (world->active[player_index] == UINT8_C(0))
    {
        ...
        return (waiting || eliminated) &&
               ...
               world->velocity_x_f32[player_index] == INT32_C(0) &&
               world->velocity_y_f32[player_index] == INT32_C(0) &&
               world->shield_recoil_x_f32[player_index] ==
                   INT32_C(0) &&
    ...
    if (grounded != UINT8_C(0))
    {
        const int ground_falcon_dive_start =
            snapshot_action_is_ground_falcon_dive_start(
                action,
                world->action_ticks[player_index]);
        ...
        const int falcon_dive_capture =
            snapshot_action_is_falcon_dive_capture_holder(
                action,
                world->hitlag_resume_action[player_index]);
        const int ground_falcon_kick =
            snapshot_action_is_ground_falcon_kick(action);
        ...
        return support != (uint8_t)PF_M4_SURFACE_NONE &&
               ...
               !snapshot_action_is_aerial_attack(action) &&
               action !=
                   (uint8_t)PF_M4_ACTION_PROJECTILE_FIRE_AIR &&
               (!snapshot_action_is_reference_air_special(action) ||
               ...
               (world->velocity_y_f32[player_index] == INT32_C(0) ||
                 ...
                 action == (uint8_t)PF_M4_ACTION_HITLAG ||
                 action == (uint8_t)PF_M4_ACTION_HITSTUN ||
                 (snapshot_action_is_landing(action) &&
    ...
    if (action == (uint8_t)PF_M4_ACTION_HITLAG ||
        action_is_damage(action) ||
        action == (uint8_t)PF_M4_ACTION_RESET_BOUND ||
        snapshot_action_is_surface_tech(action) ||
        snapshot_action_is_surface_bounce(action))
    {
        return 1;
    }
...
{
    const falcon_special_attributes *falcon_attributes =
    ...
    if (world == NULL || falcon_attributes == NULL || ground_input == NULL ||
        ...
        (world->reference_match_input_lock_ticks != UINT8_C(0) &&
         (world->tick >
              (uint64_t)PF_M4_REFERENCE_MATCH_INPUT_LOCK_TICKS ||
          world->tick +
                  (uint64_t)world->reference_match_input_lock_ticks !=
              (uint64_t)PF_M4_REFERENCE_MATCH_INPUT_LOCK_TICKS ||
    ...
    if (world->item_position_x_f32 <
            -world->arena_half_width_f32 ||
        world->item_position_x_f32 > world->arena_half_width_f32 ||
        !pf_sim_vertical_coordinate_is_within_arena(
            world->item_position_y_f32,
            world->arena_ceiling_f32) ||
        world->item_velocity_x_f32 <
            -PF_SIM_MAX_MOTION_SPEED_F32 ||
        world->item_velocity_x_f32 >
            PF_SIM_MAX_MOTION_SPEED_F32 ||
        world->item_velocity_y_f32 <
            -PF_SIM_MAX_MOTION_SPEED_F32 ||
        world->item_velocity_y_f32 >
            PF_SIM_MAX_MOTION_SPEED_F32 ||
        ...
        (world->item_state == (uint8_t)PF_M4_ITEM_STATE_INACTIVE &&
         (world->item_position_x_f32 != INT32_C(0) ||
          world->item_position_y_f32 != INT32_C(0) ||
          world->item_velocity_x_f32 != INT32_C(0) ||
          world->item_velocity_y_f32 != INT32_C(0) ||
    ...
    if (world->projectile_position_x_f32 <
            -world->arena_half_width_f32 ||
        ...
        (world->projectile_state ==
         ...
         (world->projectile_position_x_f32 != INT32_C(0) ||
          world->projectile_position_y_f32 != INT32_C(0) ||
          world->projectile_velocity_x_f32 != INT32_C(0) ||
          world->projectile_velocity_y_f32 != INT32_C(0) ||
         ...
         ++player_index)
    {
        ...
        else
        {
                 ...
                 ++stale_index)
            {
                ...
                if ((stale_index < (uint32_t)stale_move_count &&
                     (move_id == UINT8_C(0) ||
                      stale_move_id_for_action(move_id) !=
                          move_id)) ||
        ...
        if (player_index < (uint32_t)world->player_count)
        {
            ...
            const uint16_t shield_stun =
                world->shield_stun_ticks[player_index];
            const float shield_health =
                world->shield_health_f32[player_index];
            const uint8_t resume_action =
                world->hitlag_resume_action[player_index];
            const int hitstun_is_memory =
                hitstun != UINT16_C(0) &&
                snapshot_hitstun_is_memory(
                    action,
                    world->last_hit_sequence[player_index]) &&
                !(action == (uint8_t)PF_M4_ACTION_HITLAG &&
                  (action_is_damage(resume_action) ||
                   resume_action ==
                       (uint8_t)PF_M4_ACTION_RESET_BOUND ||
                   snapshot_action_is_surface_bounce(
                       resume_action)));
            ...
            if (world->active[player_index] > UINT8_C(1) ||
                ...
                (resume_action != UINT8_C(0) &&
                 !action_is_canonical(resume_action)) ||
                world->rebound_duration_ticks[player_index] >
                    UINT16_C(640) ||
                ...
                world->crouch_pass_pending_ticks[player_index] >
                    (uint8_t)(ground_input->crouch_pass_delay_ticks +
                              UINT16_C(1)) ||
                (world->crouch_pass_pending_ticks[player_index] !=
                     UINT8_C(0) &&
                 world->action_state[player_index] !=
                ...
                (world->ecb_bottom_lock_ticks[player_index] == UINT8_C(0) &&
                 world->ecb_locked_bottom_y_f32[player_index] != INT32_C(0)) ||
                (world->ecb_bottom_lock_ticks[player_index] != UINT8_C(0) &&
                 (world->ecb_locked_bottom_y_f32[player_index] <
                      -world->arena_ceiling_f32 ||
                  world->ecb_locked_bottom_y_f32[player_index] >
                ...
                ((action == (uint8_t)PF_M4_ACTION_HITLAG) !=
                 (hitlag > UINT16_C(0))) ||
                (hitlag > UINT16_C(0) &&
                 !snapshot_action_is_ground_attack(
                     resume_action) &&
                 resume_action !=
                 ...
                 !snapshot_action_is_aerial_attack(
                     resume_action) &&
                 resume_action !=
                 ...
                 resume_action !=
                     (uint8_t)PF_M4_ACTION_FALCON_KICK_LANDING &&
                 !snapshot_action_is_throw(resume_action) &&
                 resume_action != (uint8_t)PF_M4_ACTION_GRABBED &&
                 !action_is_damage(resume_action) &&
                 resume_action !=
                     (uint8_t)PF_M4_ACTION_SHIELD_STUN &&
                ...
                (hitlag == UINT16_C(0) &&
                 ...
                 action != (uint8_t)PF_M4_ACTION_RESET_BOUND &&
                 !snapshot_action_is_surface_bounce(action)) ||
                (action_is_ground_damage(action) &&
                 (world->grounded[player_index] == UINT8_C(0) ||
                  world->support[player_index] ==
                ...
                (shield_stun > UINT16_C(0) &&
                 ...
                 (action != (uint8_t)PF_M4_ACTION_HITLAG ||
                  resume_action !=
                      (uint8_t)PF_M4_ACTION_SHIELD_STUN)) ||
                (shield_health == UINT32_C(0) &&
                 !snapshot_action_is_shield_break(action) &&
                 action != (uint8_t)PF_M4_ACTION_SHIELD &&
                 action != (uint8_t)PF_M4_ACTION_SHIELD_STUN &&
                 (action != (uint8_t)PF_M4_ACTION_HITLAG ||
                  (resume_action !=
                ...
                ((action == (uint8_t)PF_M4_ACTION_KNOCKDOWN ||
                  ...
                  snapshot_action_is_throw(action) ||
                ...
                (world->damage_jump_buffer_ticks[player_index] !=
                 ...
                 !(action == (uint8_t)PF_M4_ACTION_HITLAG &&
                   (action_is_damage(resume_action) ||
                    snapshot_action_is_surface_bounce(
                        resume_action)))) ||
                !player_state_consistent(world, player_index))
            {
                return PF_STATUS_INVALID_STATE;
            }
                     ...
                     (uint8_t)PF_M4_ACTION_LEDGE_ATTACK))
            {
                ...
                ledge_claims |= ledge_bit;
                 ...
                 world->position_x_f32[player_index] != INT32_C(0) ||
                 world->position_y_f32[player_index] != INT32_C(0) ||
                 world->velocity_x_f32[player_index] != INT32_C(0) ||
                 world->velocity_y_f32[player_index] != INT32_C(0) ||
                 ...
                 world->horizontal_input_direction[player_index] !=
                     INT8_C(0) ||
                 world->damage_f32[player_index] != UINT32_C(0) ||
                 world->knockback_velocity_x_f32[player_index] !=
                     INT32_C(0) ||
                 world->knockback_velocity_y_f32[player_index] !=
                     INT32_C(0) ||
         ...
         ++player_index)
    {
        ...
        if (target_slot != UINT8_C(0))
        {
            ...
            if (target_index == player_index ||
                ...
                (!snapshot_action_is_normal_grab_holder(
                     world->action_state[player_index],
                     world->hitlag_resume_action[player_index]) &&
                 !snapshot_action_is_falcon_dive_capture_holder(
                     world->action_state[player_index],
                     world->hitlag_resume_action[player_index]) &&
                 !snapshot_action_is_throw_holder(
                     world->action_state[player_index],
                     world->hitlag_resume_action[player_index])) ||
                world->grab_owner_slot[target_index] !=
                    (uint8_t)(player_index + UINT32_C(1)) ||
                !snapshot_action_is_grabbed(
        ...
        else if (snapshot_action_is_normal_grab_holder(
                     world->action_state[player_index],
                     world->hitlag_resume_action[player_index]))
        {
            return PF_STATUS_INVALID_STATE;
        ...
        else if (snapshot_action_is_grabbed(
                     world->action_state[player_index],
                     world->hitlag_resume_action[player_index]) ||
                 world->grab_escape_ticks[player_index] != UINT16_C(0))
        {
    ...
    pf_sim_identity *out_identity)
{
    ...
    out_identity->arena_half_width_f32 =
        sim->world.arena_half_width_f32;
    out_identity->arena_ceiling_f32 =
        sim->world.arena_ceiling_f32;
    ...
    pf_mut_bytes *destination)
{
    ...
    if (!snapshot_content_state_consistent(
            &sim->content,
            &sim->world))
    {
        return PF_STATUS_INVALID_STATE;
         ...
         ++player_index)
    {
        if (candidate.source_submotion[player_index] !=
            snapshot_canonical_source_submotion(
                &candidate,
                player_index) ||
            candidate.source_animation_frame_f32[player_index] !=
                snapshot_canonical_source_animation_frame_f32(
                    &candidate,
                    player_index) ||
            candidate.source_animation_rate_f32[player_index] !=
                snapshot_canonical_source_animation_rate_f32(
                    &candidate,
                    player_index) ||
            candidate.fall_animation_blend_f32[player_index] !=
                snapshot_canonical_fall_animation_blend_f32(
                    &candidate,
                    player_index) ||
            candidate.fall_animation_target_switched[player_index] !=
                snapshot_canonical_fall_animation_target_switched(
                    &candidate,
                    player_index))
        {

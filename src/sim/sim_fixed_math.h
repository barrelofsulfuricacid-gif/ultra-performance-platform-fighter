...
extern const uint16_t atan_turn_table[65];

static inline uint32_t u64_sqrt(uint64_t value)
{
...
static inline uint16_t fixed_atan_octant_turn(
    uint32_t numerator,
    uint32_t denominator)
{
    ...
    const uint32_t lower = atan_turn_table[index];
    const uint32_t upper = atan_turn_table[
...
static inline uint16_t fixed_atan2_turn(int32_t y, int32_t x)
{
    ...
    octant = absolute_x >= absolute_y
                  ? fixed_atan_octant_turn(absolute_y, absolute_x)
                  : (uint16_t)(
                        UINT16_C(16384) -
                        fixed_atan_octant_turn(absolute_x, absolute_y));
    if (x >= INT32_C(0))
    {
        angle = y >= INT32_C(0)
                    ? (uint32_t)octant

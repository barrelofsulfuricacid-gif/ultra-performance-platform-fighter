    ...
    int32_t ecb_locked_bottom_y_q16)
{
    ...
    const int pose_available = pf_m4_falcon_reference_hsd_ecb_pose(
...
int main(int argc, char **argv)
{
        ...
        strcmp(argv[5], "--max-ticks") != 0)
    {
        (void)fprintf(
            stderr,
            "usage: pf_m4_slippi_match_trace --stage-id ID --seed SEED "
            "--max-ticks TICKS\n");
        return 1;
    }
    errno = 0;
    ...
    status = pf_m4_start_reference_match(sim);
    if (status != PF_STATUS_OK)
    {

...
static int pf_native_initialize_sim(pf_native_session *session)
{
    ...
    status = pf_m4_reference_stage_content(
    ...
    status = pf_m4_make_content_view(
        &session->content,
        &session->content_view);
    if (status != PF_STATUS_OK)
    {
...
static int pf_native_tick(pf_native_session *session)
{
    ...
    status = pf_m4_inspect(session->sim, &inspection);
    if (status != PF_STATUS_OK)
    {
        return 0;
    }
    ...
    int collision_inspector)
{
    ...
    if (pf_m4_reference_stage_geometry_line_count(
            PF_M4_REFERENCE_STAGE_BATTLEFIELD,
    ...
    {
        pf_m4_reference_stage_line line;
        SDL_FPoint start;
        SDL_FPoint end;

        if (pf_m4_reference_stage_geometry_line(
                PF_M4_REFERENCE_STAGE_BATTLEFIELD,
                index,
                &line) != PF_STATUS_OK)
        {
        ...
        {
            switch ((pf_m4_reference_stage_line_kind)line.kind)
            {
...
static void pf_native_draw_blast_inset(
    SDL_Renderer *renderer,
    const pf_m4_stage_inspection *stage,
    int width)
{
    ...
    pf_native_session *session)
{
    ...
    if (pf_m4_inspect(session->sim, &inspection) != PF_STATUS_OK ||

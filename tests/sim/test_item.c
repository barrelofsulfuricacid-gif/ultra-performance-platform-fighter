...
{
    struct content disabled;
    struct content enabled;
    ...
    if (!expect_status(
        ...
        !expect_status(
            make_content_view(&disabled, &disabled_view),
            PF_STATUS_OK,
            "disabled-view") ||
...
static int run_ucf_cardinal_item_throw_contract(
    const struct content *ucf_content,
    const pf_content_view *ucf_view)
{
    ...
    const pf_input_raw_pad horizontal_cardinal_boundary = {
        ...
        INT8_C(0),
        INT8_C(0)};
    struct content generic_content = *ucf_content;
    ...
    if (!expect_status(
            make_content_view(&generic_content, &generic_view),
            PF_STATUS_OK,
    ...
    const pf_content_view *view)
{
    ...
    pf_sim *sim = NULL;
    pf_tick_result result;
    struct inspection inspection = {0};
    const test_command attack = {
        INT16_MAX,
        INT16_C(0),
        PF_INPUT_BUTTON_ATTACK,
...
static int run_reset_contract(
    const struct content *base_content)
{
...
int main(void)
{
    ...
    (void)printf(
        "m4-item=pass content_schema=%u state_schema=%u save_bytes=1800 "

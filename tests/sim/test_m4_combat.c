    ...
    const pf_content_view *view)
{
    ...
    if (!initialize_sim(
        ...
        source_inspection.players[0].smash_charge_ticks != UINT16_C(1))
    {
    ...
    if (!expect_status(
            ...
            "smash-charge-save") ||
        destination.size != save_size ||
        memcmp(save_bytes, "PFSAVE48", (size_t)8) != 0)
    {
    ...
    for (tick = UINT32_C(1);
         tick < (uint32_t)content->fighter.smash_charge_max_ticks;
         ++tick)
    {
        if (!step_reaction_duel(
                source,
    ...
    if (trade_source_mask != UINT8_C(3) ||
        source_inspection.players[0].damage_q16 != maximum_damage_q16 ||
        source_inspection.players[1].damage_q16 != maximum_damage_q16 ||
        source_inspection.players[0].last_hit_attacker != UINT8_C(1) ||
        source_inspection.players[1].last_hit_attacker != UINT8_C(0) ||
        source_inspection.players[0].smash_charge_ticks != UINT16_C(0) ||
        source_inspection.players[1].smash_charge_ticks != UINT16_C(0))
    {
        return fail("smash-charge-slot-independent-trade");
    }
    return 1;
}
    ...
    const pf_content_view *view)
{
    ...
    if (!step_reaction_duel(
            ...
            INT16_C(0),
            (int16_t)(
                content->fighter.axis_dead_zone - UINT16_C(1)),
            PF_INPUT_BUTTON_ATTACK,
            UINT16_C(0),
            INT16_C(0),
            INT16_C(0),
    ...
    one_tick_damage_q16 =
        ...
        (uint32_t)(
            ...
            ((uint64_t)UINT32_C(65536) *
             (uint64_t)content->fighter.smash_charge_max_ticks));
    if (!step_duel(
            standing,
            INT16_MAX,
            PF_INPUT_BUTTON_ATTACK,
    ...
    const pf_content_view *view)
{
    ...
    const uint32_t shield_damage =
        (uint32_t)(
            ((uint64_t)content->fighter.jab_damage_q16 *
             (uint64_t)content->fighter
                 .shield_damage_multiplier_q16) >>
            16U);
    ...
    if (!start_held_shield_block(
        ...
        light_inspection.players[1].shield_stun_ticks == UINT16_C(0) ||
        light_inspection.players[1].velocity_x_q16 <= INT32_C(0) ||
        test_last_result.event_count != UINT8_C(1) ||
        test_last_result.events[0].type !=
            (uint16_t)PF_SIM_EVENT_SHIELD_BLOCK ||
        test_last_result.events[0].value_q16 != shield_damage)
    ...
    light_stun = light_inspection.players[1].shield_stun_ticks;
    ...
    if (!start_held_shield_block(
        ...
        dense_inspection.players[1].shield_stun_ticks != light_stun ||
        dense_inspection.players[1].velocity_x_q16 <= INT32_C(0) ||
        dense_inspection.players[1].velocity_x_q16 >= light_pushback ||
        test_last_result.event_count != UINT8_C(1) ||
        test_last_result.events[0].type !=
            (uint16_t)PF_SIM_EVENT_SHIELD_BLOCK ||
        test_last_result.events[0].value_q16 != shield_damage)
    ...
    if (!start_window_shield_block(
        ...
        window_inspection.players[1].shield_stun_ticks != light_stun ||
        test_last_result.event_count != UINT8_C(1) ||
        test_last_result.events[0].type !=
            (uint16_t)PF_SIM_EVENT_SHIELD_BLOCK ||
        test_last_result.events[0].value_q16 != shield_damage)
    ...
    pf_m4_inspection *out_inspection)
{
    ...
    return advance_missed_tech_to_down_wait(
               content,
               sim,
               out_inspection) &&
    ...
    pf_m4_inspection *out_inspection)
{
    ...
    const int32_t expected_speed =
        (int32_t)direction * content->fighter.getup_roll_speed_q16;
    ...
    if (!step_reaction_duel(
        ...
        out_inspection->players[1].prone_orientation !=
            prone_orientation ||
        out_inspection->players[1].velocity_x_q16 !=
            (timing->movement_begin_tick == UINT16_C(1)
                 ? expected_speed
                 : INT32_C(0)) ||
        out_inspection->players[1].invulnerable !=
        ...
        if (tick < content->fighter.getup_roll_ticks)
        {
            ...
            if (out_inspection->players[1].action_state !=
                ...
                out_inspection->players[1].prone_orientation !=
                    prone_orientation ||
                out_inspection->players[1].velocity_x_q16 !=
                    (action_frame >= timing->movement_begin_tick
                         ? expected_speed
                         : INT32_C(0)) ||
                out_inspection->players[1].invulnerable !=
                    (action_frame >=
    ...
    const pf_content_view *view)
{
    ...
    for (route = UINT32_C(0); route < UINT32_C(4); ++route)
    {
        if (!initialize_sim(
                &storage[route],
                view,
...
int main(void)
{
    ...
    pf_m4_content invalid_light_shield_pushback_content;
    ...
    invalid_light_shield_depletion_content.fighter
        ...
        UINT32_C(1);
    invalid_light_shield_pushback_content = content;
    invalid_light_shield_pushback_content.fighter
    ...
    if (!expect_status(
        ...
        !expect_status(
            pf_m4_validate_content(
                &invalid_light_shield_pushback_content),
            PF_STATUS_INVALID_CONFIG,
            "reject-invalid-light-shield-pushback") ||
        !expect_status(
            pf_m4_validate_content(

...
- [x] Pass Windows Release 36/36 in 2.51 seconds with bounded two-way test
  scheduling and WSL Release 36/36 in 3.04 seconds after the shield-break ECB
  import. The latest stored gate covers all 21 domains / 117 cases plus replay
  in 0.790 seconds on Windows and 0.842 seconds in WSL. WSL ASan/UBSan passes
  25/25 in 9.85 seconds. The rebuilt Emscripten target and real in-app-browser
...
- [ ] Continue the remaining edge-acquisition/action-specific ECB audit. The
  ...
  floor-contact tracks, and both complete 26-frame DownBound ECB tracks are
  closed. Getup options and broader per-action ECB evolution remain open;

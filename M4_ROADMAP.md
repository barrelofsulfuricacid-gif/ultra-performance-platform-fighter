...
| --- | --- | --- |
| Fast stored equivalence | done for twenty-one domains | The registry contains 117 cases plus deterministic replay. The pose domains hash 1,290 production poses: 714 common/ledge/guard, 33 Turn/TurnRun, 186 ordinary-airborne, 79 Pummel/CaptureWaitHi/CaptureDamageHi, and 278 CrouchWait/Appeal poses. Raptor Boost adds 502 field-masked numeric samples, Falcon Kick adds 399, natural movement adds 520, the five aerial-landing routes add 685, and passive shield break adds 500. Independent domains run concurrently while output retains manifest order. The current full gate takes 0.833 seconds on Windows and 1.001 seconds in WSL, below the two-second budget. Eight fractional ground-loop, three direct Wait, and nine pressure-derived GuardSetOff HSD observations run in the shared C pose gate because they validate the generic source evaluator rather than duplicating production pose data in the registry. |
...
| Falcon bounded hurt poses | common, base Wait, guard, turn, ledge, ordinary airborne, pummel/capture, crouch-wait, taunt, dynamic ground-loop, GuardSetOff, and shield-break poses imported | The common-state domain covers 255 source poses. Two independent clean-entry captures qualify Wait's 54 direct frames 6-59 (108 observations / 1,188 capsules) within one Q16 unit; frames 0-5 remain an explicit SquatRv-to-Wait blend gap and terminal Wait2/Wait3 selection remains an HSD-RNG gap. GuardOn/Guard/GuardOff add 25 poses / 275 capsules from two semantically identical fresh captures; GuardOn uses its hidden eight-update manual blend clock, Guard is the fixed terminal pose, and GuardOff uses ordinary frames 0-15. Turn/TurnRun add 33 unique poses / 363 capsules, including source frame zero and the seven-observation frame-9 freeze. The single update where gameplay facing has flipped but display bones retain their prior facing is derived from the existing TurnRun tick/facing/direction tuple and now drives combat plus inspection without new snapshot state. Three stored phase cases cover pre-flip frame 9, pending-display frame 9, and resumed frame 10. Two byte-identical no-fast-forward captures add 434 quick/slow ledge-option poses (4,774 capsules). Two independent unlimited headless captures add all 186 ordinary-airborne poses / 2,046 capsules. The natural paired route adds all 24 CatchAttack, 35 CaptureWaitHi, and 20 CaptureDamageHi poses plus the measured 53,806-Q16 capture anchor. Two independent grounded-loop captures add all 158 CrouchWait and both 60-frame Appeal orientations (278 poses / 3,058 capsules). All four velocity-driven WalkSlow/Middle/Fast and Run motions resolve fractional poses from the compact shared Q16 HSD evaluator: each of two independent captures contributes 131 live samples / 1,441 capsules agreeing with Dolphin within 2 Q16 units, and eight stored source observations protect the C implementation. Six pressure-banded GuardSetOff captures add 18 dynamic live observations / 198 capsules plus nine stored C poses, with exact hurt and maximum one-Q16 ECB agreement. Two fresh shield-break captures add 450 Fly/DownD/StandD/Furafura observations / 4,950 capsules at maximum one Q16 unit. |
| Falcon movement and combat | partial | Captured routes include wall/ceiling response, flat-floor missed/neutral/directional techs, both Up/Down prone/getup orientations, grounded player push from both ports/directions, imported Hyrule slope/DownBound/ordinary-ledge response, the exact common-data `x480` down-input ledge rejection boundary, all eight quick/slow ledge options, exact 640/480-frame CliffWait timeout and regrab cooldown, ordinary Jump/Fall airborne animation clocks, Falcon Punch's complete ground/air clocks and qualified air-physics tail, Raptor Boost's five complete fighter routes plus its live-only native Capsule branch, all six Falcon Kick routes, and a source-qualified complete Battlefield collision/environment catalog. Production imports complete Jump/Fall, all five aerial-attack ECB bottoms, all 198 ShieldBreakFly/DownD/StandD/Furafura frames, all 25 GuardOn/Guard/GuardOff poses, both 26-frame DownBound tracks, both 70-frame DownWait loops, both neutral getups, both getup attacks, all four prone-orientation/direction roll tracks, and the complete 158-frame CrouchWait ECB cycle: 829 exact four-point ground/prone poses total. The focused DownBound route reproduces 52 poses from 600 rows in 6.6 seconds; the eight-case getup route reproduces 438 poses from 1,150 rows in 10.5-10.9 seconds using four parallel headless workers. The one-case CrouchWait repeat reproduces all 158 source-frame-indexed poses from 160 rows in 5.31 seconds warm and confirms the authoritative five-case capture semantically. Two independent 500-frame passive-depletion captures qualify the exact Fly/DownD/StandD/Furafura sequence, animated-ECB contact update, transition velocity, shield regeneration, and Furafura reset behavior. Furafura now advances its independent 100-frame source animation clock while mash inputs reduce only the daze countdown; the captured 99-to-0 wrap is exact without adding rollback state. WalkSlow/Middle/Fast and Run now derive transition and post-blend ECB/hurt geometry from one compact HSD pose. The 27-row fixed-point recurrence oracle plus five checkpoint-equivalent production ticks cover Wait-to-Walk, two ordinary gait changes, a nested gait change, and Dash-to-Run; production transition geometry stays within 4 Q15 rotation and 4 Q16 translation units. Raptor Boost plus Falcon Dive start/catch/throw and common FallSpecial now derive complete ECBs from the same DAT/HSD evaluator. Nine retained captures qualify eight motions, 733 observations, and 715 unique frames within two Q16 units. GuardSetOff now shares that evaluator and reproduces its callback-derived light/mid/dense fractional rate, receiving-pose hitlag freeze, and first resumed advance across six independent captures / 18 live samples; nine stored pose/ECB observations protect the C implementation. A fresh 165-frame natural Falcon Dive miss route passes the identical-input comparator with the documented 640-Q16 position envelope. FallSpecial additionally reproduces the decomp's velocity-selected neutral/forward/back target, switch-update double blend, stable-update single blend, and persistent bottom-lock state. The former Raptor/Falcon Dive/FallSpecial authored ECB arrays are removed. Broader action-specific ECB and special-action acquisition coverage stay open. |
...
- [x] Qualify only the source-direct interval. SquatRv-to-Wait performs an
  ordinary six-frame HSD blend, so frames 0-5 are not misrepresented as raw
  Wait. Across frames 6-59, the generic verifier checks 108 observations /
  1,188 hurt capsules plus ECB and reports maximum one-Q16 coordinate error.
- [x] Reuse the canonical source-submotion/frame/rate fields and shared compact
  HSD evaluator. Ground idle advances Wait at rate 1, combat and collision use
  dynamic hurt/ECB geometry from frame 6, and three generated source
  observations protect first-unblended, middle, and terminal poses without
  copying a production pose table.
- [x] Pass Windows Release 40/40, WSL Release 40/40, and WSL ASan/UBSan 26/26.
  The 21-domain / 117-case stored-plus-replay gate passes in 0.833 seconds on
  Windows and 1.001 seconds in WSL. The idle cursor changes intermediate replay
  hashes as intended while the tick-240 final-state and event digests remain
  unchanged.
- [ ] Import the six-frame incoming pose blend and the process-global HSD RNG
  state/call ordering needed for Wait2/Wait3. Until then production clamps the
  last qualified base-Wait pose and makes no complete-idle equivalence claim.
...
- [x] Document clean base-Wait entry, blend-completion qualification, and the
  requirement to import the global HSD RNG lifecycle before claiming
  Wait2/Wait3 terminal behavior.

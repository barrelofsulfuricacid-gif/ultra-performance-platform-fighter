...
| --- | --- | --- |
| Fast stored equivalence | done for thirty-one registered domains plus focused Wait lifecycle | The registry contains 186 cases plus deterministic replay and passes below the two-second budget in three current isolated Windows runs: 1485.200/1706.950/1922.805 ms and three current warm WSL runs: 1096.947/1127.462/1220.305 ms. The 12-case/72-sample Turn-interrupt domain passes by itself in 304.855/307.646/347.483 ms on Windows and 376.784/415.305/458.985 ms in WSL. The pose domains hash 1,290 production poses: 714 common/ledge/guard, 33 Turn/TurnRun, 186 ordinary-airborne, 79 Pummel/CaptureWaitHi/CaptureDamageHi, and 278 CrouchWait/Appeal poses. Raptor Boost adds 502 field-masked numeric samples, Falcon Kick adds 399, natural movement adds 520, the five aerial-landing routes add 685, passive shield break adds 500, grounded slope damage adds 120, released Damage/DamageFall air-dodge IASA adds four exact action/tumble samples, Run-to-RunBrake acquisition adds 127 exact action/tick/facing/grounded samples, early Turn interrupts add 72, common special acquisition adds 188, KneeBend up-special acquisition adds 18, crouch up-special acquisition adds 106, aerial neutral-special turnaround adds 91, teeter special acquisition adds 28, and GuardOff acquisition adds 119. The eight exact acquisition domains require their production canonical digest to equal the live-source digest; two independently pinned regression digests are not accepted as equivalence. Independent domains run concurrently while output retains manifest order. Eight fractional ground-loop, three direct Wait, six SquatRv-to-Wait entry, 32 Wait2/Wait3 lifecycle, and nine pressure-derived GuardSetOff observations use shared C pose gates. The Wait lifecycle also has its own focused 440-tick/38-pose native CTest. |
| Fast live Dolphin oracle | done for the current registered domains | Registered packs use headless/null/unlimited ExiAI and checkpoint-isolated cases. A four-slot Run-to-RunBrake pack retains 127 rows across direct-down braking, radial-gate diagonal-down continuation, RunBrake-to-crouch, and locked post-TurnRun rejection. Its two row-identical captures have distinct raw artifact SHA-256 values `1d3c568f38f6dcd359e77c3b1616a6e7d81480dff4e8b3aa5262e528533fd8b9` / `e74a8c0ecc7628ba2886e7ad10b4633d2e1ad0eac5ecf6c5ec86f057a9d1ab16`, exact source/production semantic SHA-256 `dfa7be0339110c98c9107a069ef7e9751b14f2c174bd04a7e977c90ae745f6ad`, and warm times 0.541609/0.311504 seconds (6.177062/3.647950 seconds for complete lifecycles). A focused two-case/68-row damage-IASA theorem proves that released ordinary non-tumble Damage accepts a fresh L edge into AirDodge while DamageFly/DamageFall rejects it and remains tumbling for two airborne rows; its source semantic SHA is `7ce52b784989e56f7539b79dd779eed94ab41e4bcd624b980c263af0b916084b`, and warm captures take 0.587707/0.897459 seconds. The special-acquisition pack uses all sixteen immutable one-shot slots and a hidden neutral restore boundary; its two fresh 188-row captures are byte-identical and take 2.529/3.092 seconds warm (5.551/6.262 seconds for complete launch-to-cleanup lifecycles). A separate four-slot teeter pack reaches Ottotto naturally after an unrecorded low-walk setup, captures 28 rows in 0.245-0.268 seconds warm, and repeats byte-identically. The 17-case GuardOff pack uses real Falcon Jabs, starts shield on the causal attack edge, reaffirms it at the opponent's observed Jab frame, and shards at most three divergent cases across six concurrent headless workers. Its two 119-row captures are byte-identical and take 7.867/7.607 seconds warm (9.103/7.937 seconds cold). It proves the full powershield dispatcher: four specials, seven ground attacks, grab, ordinary special rejection, and ordinary/powershield jump plus spot dodge. The focused Turn/TurnRun pack captures 39 rows in 0.228 seconds warm with fast-forward and 0.565 seconds with fast-forward disabled; both produce semantic SHA `1cc3543b1363ecb5c7427c36f4d8d8a2826f9fb7c5281877f54108e1ffe281a2`. Generic label-based projection reduces the WalkFast changed-domain capture from seven cases / 423 rows / 4.820601 seconds warm to one case / 32 rows / 0.160820 seconds warm. The full common-hurt pack now owns all 16 physical boundaries / 323 rows and measures 14.10-15.05 seconds warm; the focused guard-only capture is about 0.15 seconds warm. Restored-state memory overrides are applied only after an unrecorded load boundary, so neither inputs nor direct writes can target the discarded branch. Unsafe cross-invocation observer reconnection is rejected. |
...
| Web acceptance architecture | duplicate probes removed; owner interaction gates explicit | Four source-grep verifier scripts and duplicated gameplay scenarios are deleted. The compact `web.m4_playtest` gate now protects only the real bridge ABI: exact main/C-stick/button/independent-trigger forwarding, Team Lab slot routing, and dynamic shield/hit-sphere packing. The generated Emscripten page and Chrome smoke pass with the current replay. Collision-toggle, result/rematch, and replay-navigation/fail-closed import remain honestly named owner-interaction gates instead of being overclaimed from a static DOM dump. |
| Slippi replay differential | active prior-art and prototype lane | A continuously assigned worker is surveying maintained Slippi parsers/playback tooling and public legal Falcon NTSC 1.02 corpora, then will feed recorded inputs through the source and production paths, stop at the first semantic desync, minimize its prefix, and convert confirmed gaps into focused source-qualified domains. Unsupported stages, fighters, items, or engine state must be reported rather than masked. This broad corpus lane supplements focused theorems; it does not by itself prove all simulation behavior. |

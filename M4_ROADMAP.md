...
| --- | --- | --- |
| Fast stored equivalence | done for twenty-five registered domains plus focused Wait lifecycle | The registry contains 138 cases plus deterministic replay. The pose domains hash 1,290 production poses: 714 common/ledge/guard, 33 Turn/TurnRun, 186 ordinary-airborne, 79 Pummel/CaptureWaitHi/CaptureDamageHi, and 278 CrouchWait/Appeal poses. Raptor Boost adds 502 field-masked numeric samples, Falcon Kick adds 399, natural movement adds 520, the five aerial-landing routes add 685, passive shield break adds 500, grounded slope damage adds 120, common special acquisition adds 134 exact action/tick/facing samples, teeter special acquisition adds 28, and powershield GuardOff special acquisition adds 35. Independent domains run concurrently while output retains manifest order. Eight fractional ground-loop, three direct Wait, six SquatRv-to-Wait entry, 32 Wait2/Wait3 lifecycle, and nine pressure-derived GuardSetOff observations use shared C pose gates. The Wait lifecycle also has its own focused 440-tick/38-pose native CTest. |
| Fast live Dolphin oracle | done for the current registered domains | Registered packs use headless/null/unlimited ExiAI and checkpoint-isolated cases. The special-acquisition pack uses eight immutable one-shot slots and a hidden neutral restore boundary; its two fresh 134-row captures are byte-identical and take 1.048/0.721 seconds warm. A separate four-slot teeter pack reaches Ottotto naturally after an unrecorded low-walk setup, captures 28 rows in 0.245-0.268 seconds warm, and repeats byte-identically. The five-slot powershield pack uses a real Falcon Jab, synchronizes the shield edge to the opponent's observed Jab frame, captures 35 rows in 0.605-0.649 seconds warm, and repeats byte-identically. Its negative control proves ordinary GuardOff rejects neutral B. The focused Turn/TurnRun pack captures 39 rows in 0.228 seconds warm with fast-forward and 0.565 seconds with fast-forward disabled; both produce semantic SHA `1cc3543b1363ecb5c7427c36f4d8d8a2826f9fb7c5281877f54108e1ffe281a2`. Generic label-based projection reduces the WalkFast changed-domain capture from seven cases / 423 rows / 4.820601 seconds warm to one case / 32 rows / 0.160820 seconds warm. The full common-hurt pack now owns all 16 physical boundaries / 323 rows and measures 14.10-15.05 seconds warm; the focused guard-only capture is about 0.15 seconds warm. Restored-state memory overrides are applied only after an unrecorded load boundary, so neither inputs nor direct writes can target the discarded branch. Unsafe cross-invocation observer reconnection is rejected. |
...
after closing DamageFlyTop/Roll selection, velocity-oriented Roll geometry,
mixed hit-entry ECB ownership, grounded slope launch/recontact, the non-tumble
floor-response selector, retained Damage lifecycle, and common-state special
acquisition masks, including both Ottotto phases. Numeric response remains
qualified separately.

## Completed and verified: powershield GuardOff special acquisition

- [x] Sweep pinned decomp `9509dc0` and current upstream `d882af9`.
  `ftCo_GuardOff_IASA` exposes the full special and attack dispatcher only
  while GuardOff's powershield work flag is set; ordinary GuardOff does not.
...
- [x] Qualify four positive directions and one ordinary-shield negative
  control through a physical Falcon Jab. The fixture resets both fighters'
  complete current/previous collision-position histories and applies the
  defender's shield edge relative to the opponent's observed Jab frame with
  unbatched EXI input. Two 35-row captures are byte-identical at raw SHA-256
  ...
  Warm captures take 0.649 and 0.605 seconds.
- [x] Register the generic native-CSV domain. The complete 25-domain /
  138-case plus replay gate passes in 1.194 seconds on Windows and 1.225
  seconds in WSL. Both Release suites pass 41/41 and focused WSL ASan/UBSan
...
- [x] Qualify eight actual-input cases / 134 rows: Turn rejects neutral B and
  accepts side/up/down B; SquatWait and SquatRv accept straight and radial-gate
  diagonal down B. Two fresh captures are byte-identical at raw SHA-256
  ...
  Warm capture times are 1.048 and 0.721 seconds.

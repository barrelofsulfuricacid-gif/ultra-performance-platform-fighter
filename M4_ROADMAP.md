...
- [x] Expand the single parent-closed HSD profile to 20 motions, 1,461 tracks,
  and 12,256 keys under data SHA-256
...
- [x] Qualify hurt and ECB independently. The raw HSD matrices are exact for
  ...
  The importer therefore retains one compact 98-frame live ECB profile rather
...
- [ ] Extend the same split qualification to ShieldBreakStun/Furafura. Its
  countdown timer and looping animation cursor are distinct clocks, so it is
  intentionally the next source/capture slice rather than being inferred from
  the three completed linear motions.

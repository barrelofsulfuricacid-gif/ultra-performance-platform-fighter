...
Numeric-representation note: production simulation, generated geometry, public
state, save/replay payloads, and stored-oracle numeric projections now use
IEEE-754 binary32 under ABI 6 / arithmetic version 2. The active comparison
manifests express tolerances directly in source units through `*_tolerance_f32`;
older completed sections that cite integer quantization units are retained only
as historical evidence from before the 2026-08-13 migration. Any legacy
integer-unit label remaining in this file describes the measurement scale of a
pre-migration capture or checkpoint, never an active runtime type or tolerance
schema.
...
| Web acceptance architecture | duplicate probes removed; current replay gate pinned | Four source-grep verifier scripts and duplicated gameplay scenarios are deleted. The compact `web.playtest` gate protects only the real bridge ABI: exact main/C-stick/button/independent-trigger forwarding, Team Lab slot routing, and dynamic shield/hit-sphere packing. ABI 6 now carries the 42,572-byte float32 replay with final SHA-256 `6b4364e6d2f93d5bb174193e5d09b01af367b70bce05502a14f7ff988a9531da`; the completion evidence below records a fresh Emscripten build and exact native-versus-Wasm replay comparison. Collision-toggle, result/rematch, and replay-navigation/fail-closed import remain honestly named owner-interaction gates instead of being overclaimed from a static DOM dump. |
...
### 2026-08-14 float32 migration closure

- Active source, public headers, tests, tools, generated data, and CMake contain
  no Q16, 16.16, or fixed-point identifiers. Historical documentation retains
  old measurement labels only where chronology requires them.
- Strict Release suites pass 47/47 on Windows/MSVC and 41/41 on WSL/GCC. The
  pinned Emscripten 6.0.3 build also passes with conversion warnings promoted
  to errors; that gate exposed and closed the remaining implicit integer-to-
  float paths and incorrectly typed predicate helpers.
...
- Native and WebAssembly reproduce the same 240-input replay exactly at 42,572
  bytes. Corpus/final/event SHA-256 values are
  `5b35706ece2a4bf54b2401d0257b7363e56a9182871563a463992300e1fee632`,
  `6b4364e6d2f93d5bb174193e5d09b01af367b70bce05502a14f7ff988a9531da`,
  and `bd40b6291385ada8027c37ea7c34af7cc7ef3c339a642ce53f037391b3a06f43`.
  The rebuilt client also passes the headless-Chrome smoke at ABI 6 with 72
  replay events.
...
- [x] Download and hash-pin a substantially larger public Falcon corpus before
  ...
  Falcon `.slp` files; all 782 are present in the ignored local corpus and
  recorded in a manifest with per-file SHA-256 values.
...
- [x] Expand the bounded run to the complete 782-file archive. The full manifest
  SHA-256 is `f42d437ee166b07502a7af8316aa2ee0b473ea487802b1327ae673171493b477`.
  Eight parser workers completed in 777.694 seconds with 328 anchors found,
  111 executable diagnostic comparisons, 2,475 semantic frames, 39 passes,
  7 UCF dashback boundaries, and 72 deterministic candidates. The ignored
  report SHA-256 is
  `154ae335f86fa91aa6bbc5fe0bb0a6908d594efd44f0d220bcdba27e238a6e8f`.
- [x] Fix the first source-backed replay gap without widening tolerances.
  Pinned `ftCo_KneeBend_Anim` installs Jump before the same update's Jump IASA,
  so terminal JumpSquat can enter EscapeAir and LandingFallSpecial without an
  intermediate airborne row. Slippi also proves that analog trigger travel can
  reach its endpoint one frame before the physical L/R click. The shared jump
  entry is now factored once, terminal callback ownership is projected before
  routing, and the replay adapter reserves `65535` for the digital click while
  capping unclicked analog at `65534`. The minimized replay now clears the
  wavedash boundary and reaches its later aerial-motion residual.

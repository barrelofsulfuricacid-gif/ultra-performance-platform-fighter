...
`generated/data/falcon_ntsc102_frame_data.inc` is its numeric output.
...
`generated/data/falcon_ntsc102_frame_data.inc` byte-for-byte at SHA-256
...
`default_content` consumes this typed view rather than repeating authored
...
`tools/verify_raptor_boost.sh` selects the routes directly from that pinned
...
`movement_trace` executable and the same controller normalization and
...
`generated/data/falcon_ntsc102_hit_geometry.inc`. The compact table includes
...
`Fall` after frame 99. `tools/verify_falcon_punch.sh` rebuilds the strict
...
`reference_turn_callback_facing` helper and passes it through existing

...
| Damage, knockback, hitlag, hitstun, DI/SDI | partial | The physical shield-hit subset is decomp-mapped and qualified at three pressure bands. Jab 1, jab 2, dash attack, the three tilts, the three explicit smashes, and the five directional aerials select imported per-sphere damage, angle, KBG, set-weight, BKB, and hitlag through Melee's fixed-point response. A six-case, 138-row live Falcon route now qualifies open-air self/knockback channel separation, source-order 0.051 magnitude decay, neutral/full/half and squared-projection DI, radial SDI threshold behavior, analog displacement, and C-stick ASDI priority. Every selected hitlag/launch position, both velocity channels, hitlag, and hitstun matches within 0.001 Melee units. Ground knockback/friction and wall, ceiling, floor, tech, bounce, and getup interactions remain incomplete. |
...
Together the two stored domains cover 26 cases and complete in 205.990 ms on
Windows and 285.842 ms in WSL. The live damage pack completes its 138 rows in
0.665 seconds warm and 3.438 seconds for the full process lifecycle. These
checks prove only their registered domains; ground and collision response need
new live routes before their goldens can be admitted.

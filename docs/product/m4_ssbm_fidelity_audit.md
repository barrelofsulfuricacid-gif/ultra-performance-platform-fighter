...
### 2026-08-10 static implementation checkpoint
...
previously missing jab/angled-normal branches. It is deliberately unverified:
no test, build, generator, Dolphin route, benchmark, or runtime validation was
run. Existing `equivalent` labels below therefore still mean previously
qualified scope; the new work must pass the next validation slice before any
row is promoted on its strength.
...
1. Validate the 2026-08-10 static implementation checkpoint. Its common
   damage, shield, grab/throw, input, rebound/clank, rebirth, and Falcon action
   changes are not yet executable evidence.
2. Import exact pose/hit geometry for Jab 3, rapid jab, angled normals,
   pummel/CaptureDamage, and the remaining common states; the current fallback
   envelopes are not an equivalence claim.

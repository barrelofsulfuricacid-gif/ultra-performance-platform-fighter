...
| Roll, spot dodge, air dodge buffering | partial | Production paths and per-trigger edge tracking exist. Air-dodge force, dead zone, decay, and post-dodge drift cap are imported with axis-specific unit conversion. Falcon's exact EscapeN/EscapeF/EscapeB/EscapeAir animation lengths and flags now come from the complete generated submotion catalog rather than authored constants; invulnerability, movement-command, and broader callback routes still require executable qualification. |
...
1. Route the complete imported common submotion catalog through the remaining
   ...
   callback semantics; do not re-transcribe its frame lengths.

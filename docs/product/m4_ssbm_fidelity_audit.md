...
| Attacks, grabs, throws, stale moves | partial | A pinned importer retains Falcon's complete 50-slot timing/effect/throw schema: all 48 concrete subactions plus the source-defined absence of the mid-high/mid-low forward-smash slots, with every present action flag and per-frame translation-N delta decoded from the raw animation DAT. Production routes consume exact imported timing for jab 1/2, dash attack, all tilts, all explicit smashes, all five aerials, standing/dash grab, aerial landing lag, pummel, and all four normal throws. Separately hash-pinned Dolphin captures supply 121 live active-frame rows and 250 transformed spheres plus all 612 displayed hurt-pose frames and 6,732 capsules for the 14 production normals/aerials and both grabs; simultaneous sweet/weak spheres retain independent effects. Grounded normals use their distinct decomp IASA policies: jab chain, full Wait, restricted down tilt, and forward smash without escape. Root translation drives only jab, dash attack, and forward smash because their physics callbacks consume it; the remaining normals apply source friction semantics. Standing grab uses 5/2/22 startup/active/recovery and two live spheres on executable frames 7-8, while dash grab uses its distinct 9/2/28 state and three live spheres on frames 11-12. Grab resolution filters the target's current captured pose by the executable `grabbable` bit. Throw release frames are decoded from the original DAT action scripts and throw response uses imported damage/angle/KBG/WDKB/BKB through the shared integer Melee formula. Frame lookup preserves inactive gaps, late-hit changes, aerial frame 44, and hitlag-frozen poses. Hurt capsules for actions outside those 16, special attack geometry, source-Z collision semantics, exact sphere-versus-shield intersection, and normal-throw collateral hits remain incomplete; aerial IASA also remains to be routed. The custom generic strong-aerial/special fixtures intentionally remain outside Falcon equivalence. |
...
3. Import the remaining animated hurt capsules and special hit geometry,
   qualify Z and sphere-versus-shield collision semantics, and route normal-
   throw collateral hitboxes without duplicating constants.

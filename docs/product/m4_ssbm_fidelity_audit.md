...
| Run turnaround | partial | Full reversal from terminal run retains the old facing, applies full TurnRun acceleration, freezes displayed frame 9 until velocity crosses the common 0.01 threshold, flips facing on the following physics tick, resumes through displayed frame 21, and enters the ten-tick locked run route. All 22 source poses, including frame zero, are imported and source submotion is retained. The live oracle also exposes one frame-9 update where gameplay facing has flipped but display bones still use the previous facing. Production currently mirrors the canonical pose immediately on that update, so this transient collision-facing phase remains open. |

...
| Ground and platform collision | partial | The isolated 348-frame Battlefield route matches neutral jump-through ascent, the final ordinary-airborne descending crossing before `Landing`, 0.63 Pass entry speed, and same-frame solid-floor landing. The paired 48-row / 96-lane domain matches grounded push from both directions and controller ports. The imported Hyrule line-34 slope and line-36/37 endpoint domain additionally qualifies slope-tangent landing projection, DamageFly ECB, DownBound contact/support ordering, collision-driven Fall, the exact eight-frame Fall animation loop, and ordinary ledge entry through seven `CliffCatch` frames and first `CliffWait`; its source/production digests are `0b23132b7a217ff173397faf8ac9e59169092c99095b4b4e3fbd885526b7a3f3` and `918a548c6de3a53ab04f89d8bc5232e97b298d7887dc67661f42bd02a9626d66`. General platform/edge shapes, later wait/options, and broader ECB evolution remain unqualified. |
| Ledge jump velocities | equivalent | Falcon 1.0 horizontal and 3.3 vertical attributes are mapped. |
| Other ledge actions | partial | Ordinary airborne reach consumes imported Falcon snap attributes, source-root/ECB coordinates, Hyrule endpoint/ledge flags, and current/previous positions. Production now preserves the seven-frame `CliffCatch` lifecycle, uses live-qualified absolute catch/wait roots plus decoded catch deltas, suppresses inputs during catch, enters the first exact `CliffWait` frame, and validates that state in save/hash and exclusive ledge ownership. Later wait/input timing, drop/climb/roll/attack root and invulnerability tables, regrab semantics, percent-dependent ledge tables, and complete ledge pose geometry remain incomplete. |
...
| Stage geometry, blast zones, spawns | partial | The Relay Rod laboratory remains an original test stage. A source-derived immutable Hyrule catalog now covers MapCollData lines 34-37 with world-space vertices, normals, adjacency, endpoints, and ledge flags under semantic digest `4a0dd57bb8d9532589d3ecd129213d3a0876538a2dc7f733eca6c1e73c04db9c`; only this response slice is production-routed. Complete Hyrule/Battlefield geometry, blast zones, platforms, and spawns remain incomplete. |

...
| Roll, spot dodge, air dodge buffering | partial | Production paths and per-trigger edge tracking exist. Air-dodge force, dead zone, decay, and post-dodge drift cap are imported with axis-specific unit conversion. Falcon's exact EscapeN/EscapeF/EscapeB/EscapeAir animation lengths and flags come from the generated submotion catalog, while the lossless action-script table retains their body-collision, IASA, and reverse-direction commands. Those commands are data-available but their displayed-frame mapping, invulnerability semantics, movement callbacks, and broader executable routes still require qualification before replacing runtime fields. |
| Tech and neutral getup body state | partial | The generated script decoder covers all 318 submotions and exposes raw state-2/state-0 command frames. Production tech in place and both tech rolls consume the matching 0-20 window; neutral getup validates both prone orientations and consumes their matching 0-23 window. Ceiling/wall tech, directional getup rolls, getup attacks, and their pose/movement differences remain to be split and Dolphin-qualified. |
...
| Attacks, grabs, throws, stale moves | partial | A pinned importer retains Falcon's complete 50-slot timing/effect/throw schema: all 48 concrete subactions plus the source-defined absence of the mid-high/mid-low forward-smash slots, with every present action flag and per-frame translation-N delta decoded from the raw animation DAT. Production routes consume exact imported timing for jab 1/2, dash attack, all tilts, all explicit smashes, all five aerials, standing/dash grab, aerial landing lag, pummel, all four normal throws, and all four Falcon specials. It also decodes the global `PlCo.dat` stale table exactly as 9/8/7/6/5/4/3/2/1 percent; the previous authored approximations are removed. Separately hash-pinned Dolphin captures supply transformed hit geometry and complete 11-capsule poses for those 16 actions and all 17 Falcon special subactions; simultaneous sweet/weak spheres retain independent effects, while Raptor Boost uses its separately typed six-sphere search geometry. Grounded normals use their distinct decomp IASA policies: jab chain, full Wait, restricted down tilt, and forward smash without escape. Root translation drives jab, dash attack, forward smash, Falcon Punch, Raptor Boost, Falcon Dive, and Falcon Kick where their physics callbacks consume it; the remaining normals apply source friction semantics. Standing grab uses 5/2/22 startup/active/recovery and two live spheres on executable frames 7-8, while dash grab uses its distinct 9/2/28 state and three live spheres on frames 11-12. Grab resolution filters the target's current captured pose by the executable `grabbable` bit. Throw release frames are decoded from the original DAT action scripts; their ordinary moving spheres and separate release responses use imported damage/angle/KBG/WDKB/BKB through the shared integer Melee formula. Imported hit spheres and both hurt-capsule endpoints retain source X/Y/Z, share Melee's root origin, and use one allocation-free 3D point-to-capsule predicate; inspection schema 52 exposes Z without changing the browser's 2D projection. Reference spheres also include Z in the source-metric shield predicate. Frame lookup preserves inactive gaps, late-hit changes, aerial frame 44, and hitlag-frozen poses. Common-state hurt poses, the executable's previous-to-current moving hit-capsule sweep, aerial IASA, and unsampled Falcon-special dynamic routes remain incomplete. The custom generic strong-aerial/special fixtures intentionally remain outside Falcon equivalence. |
...
1. Route the complete imported common submotion catalog through the remaining
   shield, dodge, ledge, tech, damage, item, and unaudited brake command/
...
3. Capture remaining common-state hurt poses and qualify moving hit-capsule
   sweeps, aerial IASA, and unsampled dynamic routes without duplicating
   constants.
...
evidence is 14,826 captured frames. The memory-probed routes qualify the

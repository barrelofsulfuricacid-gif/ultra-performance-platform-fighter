...
`content` contains the validated fighter and stage tables. Its canonical
...
`inspect` exposes the deterministic movement state and current stage
...
`tests/sim/test_movement.c` and `tools/verify_movement.sh` cover:
...
The `web.playtest` CTest owns the view schema and exported step/configuration

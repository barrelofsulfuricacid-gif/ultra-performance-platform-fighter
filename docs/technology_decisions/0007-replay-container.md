...
- **Status:** Accepted for replay format 1 / API schema 3
- **Date:** 2026-07-28
...
Input chunk version 1 is deliberately uncompressed. Each input remains an
explicit field-by-field little-endian 32-byte frame. Under input schema 6, the
control word contains the logical buttons plus four disjoint signed raw
PADStatus axis bytes, and the final byte carries their per-axis validity mask.
An axis without validity must encode a zero raw byte; the simulation applies
its deterministic processed-axis fallback when verifying that frame.
This provides the simple canonical oracle against which a later
delta-compressed chunk can be tested. Compression can be introduced as a new
chunk version without changing simulation input semantics.

Replay API schema 3 and verification schema 2 embed the ABI-5 tick-result
...
- Replay size (42,555 bytes) and hashes are regenerated by the
  native/WebAssembly corpus for M4 state schema 77 / save format 67 / RNG
  behavior version 2.
